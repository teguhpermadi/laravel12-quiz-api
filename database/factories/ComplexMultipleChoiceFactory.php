...
class ComplexMultipleChoiceFactory extends Factory
{
    ...
    public function configure()
    {
        return $this->afterCreating(function ($choice) {
            ...
            try {
                ...
                $text = "Complex Choice: " . substr($choice->choice, 0, 30);
                ...
                if (file_exists($filePath)) {
                    ...
                    $choice->addMedia($filePath)
                        ->toMediaCollection('answer_media');
                    
                    Log::info('Generated complex choice image added to media collection', [
                        'choice_id' => $choice->id,
                        'file_path' => $filePath
                    ]);
                } else {
                    Log::error('Failed to generate complex choice image', [
                        'choice_id' => $choice->id
                    ]);
                }
            } catch (\Exception $e) {
                Log::error('Complex Choice Media Error:', [
